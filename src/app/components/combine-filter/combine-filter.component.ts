...
@Component({
  selector: 'app-combine-filter',
  templateUrl: './combine-filter.component.html',
  styleUrls: ['./combine-filter.component.css']
})
export class CombineFilterComponent implements OnInit {
  message: string;
  ...
  protected getMeasureValueProps(): MeasureValueFilterProps {
    ...
    return {
      workspace: workspace,
      backend: backend,
      filter: this.filterValue ? this.filterValue : this.filters[0],
      onApply: this.onApplyMeasure,
      buttonTitle: this.measureTitle,
      measureIdentifier: measureLocalId("1c8bf813879f48aaa54c74dd8551b93c")
    };
  }
  ...
  protected getPivotTableProps(): PivotTableBucketProps {
    return {
      ...
      rows: this.rows,
      filters: this.filterValue ? [this.filterValue] : self.filters,
    };
  }

  protected getInsightViewProps(): InsightViewProps {
    return {
      workspace: workspace,
      backend: backend,
      insight: Ldm.Insights.VisColumn,
  ...
  public renderPivotTable() {
    ReactDOM.render(React.createElement(InsightView, this.getInsightViewProps()), this.getLineDataNode())
  }
  ngOnInit() {
    self = this;
